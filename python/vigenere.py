...
def vigenere():
    # Por questões de segurança, todo o texto é convertido para "uppercase"
    message = input("Qual a mensagem? ").upper()
    key = input("Qual a chave? ").upper()
    encrypt = input("Deseja Cifrar? Digite 'sim' ").lower()

    to_encrypt = False
    if (encrypt == "sim"):
        to_encrypt = True

    return get_cyphered_message(message, key, to_encrypt)

cyphered_message = vigenere()
